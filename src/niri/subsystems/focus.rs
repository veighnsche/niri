...
impl FocusState {
    ...
    fn compute_layer_and_layout_focus(&self, ctx: &FocusContext) -> KeyboardFocus {
        ...
        let excl_focus_on_layer = |layer: Layer| -> Option<KeyboardFocus> {
            ctx.layer_surfaces.iter().find_map(|candidate| {
                if candidate.layer == layer && candidate.is_exclusive && !candidate.is_in_backdrop {
                    Some(KeyboardFocus::LayerShell {
        ...
        F: FnOnce(&LayerSurface) -> bool,
    {
        let should_clear = self
            .layer_on_demand
            .as_ref()
            .map_or(false, |surface| !is_valid(surface));
    ...
    pub fn shortcut_inhibitors_mut(
        &mut self,
    ) -> &mut HashMap<WlSurface, KeyboardShortcutsInhibitor> {
...
mod tests {
    ...
    fn test_mru_ui_priority() {
        ...
        assert!(matches!(focus.compute_focus(&ctx), KeyboardFocus::Mru));
    }
}
