...
impl OutputSubsystem {
    ...
    pub fn previous_of(&self, current: &Output) -> Option<&Output> {
        ...
        for output in iter.by_ref() {
            if output == current {
                return prev.or_else(|| self.sorted.last()).filter(|&o| o != current);
            }
            prev = Some(output);
        }
        None
    ...
    pub fn next_of(&self, current: &Output) -> Option<&Output> {
        ...
        while let Some(output) = iter.next() {
            if output == current {
                return iter.next().or_else(|| self.sorted.first()).filter(|&o| o != current);
            }
        }
        None
    }
    ...
    pub fn lock_surface(&self, output: &Output) -> Option<&smithay::wayland::session_lock::LockSurface> {
    ...
    pub fn find_output_needing_redraw(&self, check: impl Fn(&Output, &super::super::OutputState) -> bool) -> Option<Output> {
        self.state.iter()
    ...
    pub fn update_backdrop_color(&mut self, output: &Output, color: smithay::backend::renderer::Color32F) -> bool {
        if let Some(state) = self.state.get_mut(output) {
