//! Golden snapshot tests for layout regression testing.
//!
//! # ⚠️⚠️⚠️ GOLDEN PROTECTION — READ BEFORE MODIFYING ⚠️⚠️⚠️
//!
//! These tests compare your code against **GOLDEN BASELINE SNAPSHOTS**.
//! Golden snapshots capture the MAIN BRANCH behavior — the correct behavior.
//!
//! ## 🚫 NEVER RUN `cargo insta accept` ON THESE TESTS! 🚫
//!
//! If tests fail, it means YOUR CODE changed behavior. The snapshots are correct.
//!
//! ## If tests fail:
//!
//! 1. **DO NOT** run `cargo insta accept` — this would corrupt the golden baseline
//! 2. **DO NOT** accept `.snap.new` files — they contain your broken output
//! 3. **FIX YOUR CODE** — your refactor introduced a regression
//! 4. Run `cargo xtask golden-sync clean` to remove `.snap.new` files
//!
//! ## Golden Snapshot Provenance
//!
//! - **Source**: `golden-snapshots` branch (derived from `main`)
//! - **Generated by**: `cargo xtask golden-sync generate`
...
//! cargo xtask golden-sync pull     # Pull golden snapshots
//! cargo xtask golden-sync clean    # Remove .snap.new files
//! cargo test --lib golden          # Run golden tests
//! ```
//!
//! ---
//! TEAM_004: Initial infrastructure (on wrong branch)
