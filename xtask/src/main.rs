...
//!
//! Golden snapshots capture the MAIN BRANCH behavior for regression testing.
//! Refactor branches compare their output against these golden snapshots.
//!
//! ### Commands:
//!
//! - `cargo xtask golden-sync generate` - Generate fresh snapshots from golden branch
//! - `cargo xtask golden-sync pull` - Pull existing snapshots from golden branch
//! - `cargo xtask golden-sync status` - Show sync status
//!
//! ### Full Workflow:
//!
//! 1. Run `cargo xtask golden-sync generate` (switches to golden branch, generates, copies back)
//! 2. Run `cargo test --lib golden` to compare your code against golden behavior
//! 3. If tests fail → your refactor changed behavior!
...
#[derive(Subcommand)]
enum Commands {
    /// Sync golden snapshots between branches
    GoldenSync {
        #[command(subcommand)]
        action: GoldenSyncAction,
    },
}

#[derive(Subcommand)]
enum GoldenSyncAction {
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::GoldenSync { action } => match action {
...
const GOLDEN_BRANCH: &str = "golden-snapshots";
const SNAPSHOTS_PATH: &str = "src/layout/tests/snapshots";
...
fn golden_sync_generate(dry_run: bool) -> Result<()> {
    ...
    for entry in std::fs::read_dir(from).context(format!("Failed to read {from}"))? {
        ...
        if path.extension().map_or(false, |ext| ext == "snap") {
            ...
            std::fs::copy(&path, &dest)
                .context(format!("Failed to copy {:?}", path))?;
            count += 1;
        }
    }
...
fn golden_sync_pull(dry_run: bool) -> Result<()> {
